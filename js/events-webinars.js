(function($){'use strict';
  ...
  function initEventTabs() {
    ...
    var $upcoming = $('.rego_ev_upcoming_panel');
    var $demand = $('.rego_ev_demand_panel');

    if ($demand.length) $demand.hide();

    $tabs.on('click', function(e) {
      e.preventDefault();
      $tabs.removeClass('rego_ev_tab_active');
      $(this).addClass('rego_ev_tab_active');

      var target = $(this).data('tab');
      if (target === 'upcoming') {
        if ($upcoming.length) $upcoming.show();
        if ($demand.length) $demand.hide();
      } else if (target === 'demand') {
        if ($upcoming.length) $upcoming.hide();
        if ($demand.length) $demand.show();
      }

      var headerH = parseInt(getComputedStyle(document.documentElement).getPropertyValue('--rego-header-h')) || 72;
      var $list = $('.rego_ev_event_list');
      if ($list.length && $list.offset().top > $(window).height()) {
        $('html,body').animate({scrollTop: $list.offset().top - headerH}, 440);
      }
    });

    $tabs.first().addClass('rego_ev_tab_active');
  }
