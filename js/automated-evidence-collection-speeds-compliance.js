(function($){'use strict';
  ...
  function initAecSlider() {
    var $slider = $('.rego_aec_slider');
    if (!$slider.length) return;
    var $slides = $slider.find('.rego_aec_slide');
    var $dotsWrap = $slider.find('.rego_aec_slider_dots');
    if (!$slides.length) return;
    var current = 0, total = $slides.length, timer = null;

    $dotsWrap.empty();
    for (var i = 0; i < total; i++) {
      $dotsWrap.append('<button class="rego_aec_slider_dot" type="button" data-index="' + i + '" aria-label="Slide ' + (i+1) + '"></button>');
    }
    var $dots = $dotsWrap.find('.rego_aec_slider_dot');

    function go(i) {
      current = (i + total) % total;
      $slides.removeClass('rego_aec_slide_active');
      $slides.eq(current).addClass('rego_aec_slide_active');
      $dots.removeClass('rego_aec_slider_dot_active');
      $dots.eq(current).addClass('rego_aec_slider_dot_active');
    }
