...
(function ($) {
  ...
  ---------------------------------------------------------------- */
  function initReveal() {
    // Only animate smaller child elements, never the outer layout wrapper
    var $els = $('.rego_eula_section, .rego_eula_related_card');
    ...
    // Safety: force-reveal anything still hidden after 800ms
    setTimeout(function () {
      $els.filter(':not(.rego_revealed)').addClass('rego_revealed');
    }, 800);
  }
