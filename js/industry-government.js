(function($){'use strict';
  ...
  function initSuccessSlider() {
    var $slider = $('#rego_igv_success_slider');
    if (!$slider.length) return;

    var $slides = $slider.find('.rego_igv_slide');
    var $dots   = $slider.find('.rego_igv_slider_dot');
    ...
    function show(idx) {
      ...
      $slides.removeClass('rego_igv_slide_active').eq(idx).addClass('rego_igv_slide_active');
      $dots.removeClass('rego_igv_slider_dot_active').attr('aria-selected', 'false')
           .eq(idx).addClass('rego_igv_slider_dot_active').attr('aria-selected', 'true');
      current = idx;
    }
  ...
  function initAccordion() {
    var $acc = $('#rego_igv_accordion');
    if (!$acc.length) return;

    var $vizItems = $('.rego_igv_optviz_item');

    function showViz(target) {
      if (target === null || target === undefined) return;
      $vizItems.removeClass('rego_igv_optviz_active').attr('aria-hidden', 'true');
      $vizItems.filter('[data-target="' + target + '"]')
               .addClass('rego_igv_optviz_active')
               .attr('aria-hidden', 'false');
    }

    $acc.on('click', '.rego_igv_acc_head', function() {
      var $head = $(this);
      var $item = $head.closest('.rego_igv_acc_item');
      var willOpen = !$item.hasClass('rego_igv_acc_open');

      $acc.find('.rego_igv_acc_item').removeClass('rego_igv_acc_open');
      $acc.find('.rego_igv_acc_head').attr('aria-expanded', 'false');

      if (willOpen) {
        $item.addClass('rego_igv_acc_open');
        $head.attr('aria-expanded', 'true');
        showViz($item.attr('data-target'));
      }
    });

    var $initial = $acc.find('.rego_igv_acc_item.rego_igv_acc_open').first();
