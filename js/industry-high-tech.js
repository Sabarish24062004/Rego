(function($){'use strict';
  ...
  function initSuccessSlider() {
    var $slider = $('#rego_iht_success_slider');
    if (!$slider.length) return;

    var $slides = $slider.find('.rego_iht_slide');
    var $dots   = $slider.find('.rego_iht_slider_dot');
    ...
    function show(idx) {
      ...
      $slides.removeClass('rego_iht_slide_active').eq(idx).addClass('rego_iht_slide_active');
      $dots.removeClass('rego_iht_slider_dot_active').attr('aria-selected', 'false')
           .eq(idx).addClass('rego_iht_slider_dot_active').attr('aria-selected', 'true');
      current = idx;
    }
  ...
  function initAccordion() {
    var $acc = $('#rego_iht_accordion');
    if (!$acc.length) return;

    var $vizItems = $('.rego_iht_scalviz_item');

    function showViz(target) {
      if (target === null || target === undefined) return;
      $vizItems.removeClass('rego_iht_scalviz_active').attr('aria-hidden', 'true');
      $vizItems.filter('[data-target="' + target + '"]')
               .addClass('rego_iht_scalviz_active')
               .attr('aria-hidden', 'false');
    }

    $acc.on('click', '.rego_iht_acc_head', function() {
      var $head = $(this);
      var $item = $head.closest('.rego_iht_acc_item');
      var willOpen = !$item.hasClass('rego_iht_acc_open');

      $acc.find('.rego_iht_acc_item').removeClass('rego_iht_acc_open');
      $acc.find('.rego_iht_acc_head').attr('aria-expanded', 'false');

      if (willOpen) {
        $item.addClass('rego_iht_acc_open');
        $head.attr('aria-expanded', 'true');
        showViz($item.attr('data-target'));
      }
    });

    var $initial = $acc.find('.rego_iht_acc_item.rego_iht_acc_open').first();
